...
class Cat(Animal):
    def __init__(self,name,breed,diet,no_brainscells,fur_colour):
        super().__init__(name,breed,diet)
...
while True:
    ...
    if choice == 1:
        ...
        name=input("pls enter its name:")
        breed=input("pls enter the breed:")
        diet=input("pls enter its diet:")
        
        if animal_type=="dog":
            pedigree=input("is it a pedigree dog:")
            rabies_vaccinated=input("is it vaccinated against rabies:")
            animal=Dog(name,breed,diet,pedigree,rabies_vaccinated)
        
        elif animal_type == "cat":
            no_braincells = int(input("Number of braincells: "))
            fur_colour = input("Fur colour: ")
            animal = Cat(name,breed,diet,no_braincells,fur_colour) 
        ...
        name= input("pls enter name of animal you want to modify:")
        if name in animals_vet:
            new_diet=input(f"pls enter new diet for {name}:")
            animals_vet[name].diet=new_diet
